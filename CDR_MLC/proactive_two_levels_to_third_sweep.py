...
from proactive_meta_stacked_cdr_mlc import (
    ProactiveMetaStackConfig,
    fit_proactive_meta_stacker,
    predict_all as predict_proactive_meta,
)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", type=Path,
        default=root / "outputs/proactive_two_levels_to_third",
    )
    parser.add_argument(
    ...
    proactive_config = ProactiveMetaStackConfig(
        window=args.window,
        congestion_window=args.congestion_window,
        candidate_features=tuple(args.congestion_features),
    ...
    for scenario in args.scenarios:
        ...
        proactive_result = predict_proactive_meta(
            proactive_model, target
        )
        ...
        audits[scenario] = {
            ...
            "proactive_partition_rows": (
                proactive_model["partition_rows"]
            ),
            "proactive_selected_features": list(
                proactive_model["selected_proactive_features"]
            ),
            "proactive_trend_ranking": (
                proactive_model["proactive_trend_ranking"]
            ),
            "proactive_leakage_control": (
                proactive_model["leakage_control"]
            ),
            ...
            "proactive_selected_meta_variant": (
                proactive_model["selected_meta_variant"]
            ),
            "proactive_selected_meta_confidence": (
                proactive_model["selected_meta_confidence"]
            ),
        }

    summary = pd.DataFrame(rows).sort_values(["scenario", "method"])
    summary.to_csv(
        args.output / "proactive_two_levels_to_third_summary.csv", index=False
    )
    manifest = {
        ...
        "proactive_meta_config": asdict(proactive_config),
        "proactive_protocol": {
    ...
    (args.output / "proactive_two_levels_to_third_manifest.json").write_text(
