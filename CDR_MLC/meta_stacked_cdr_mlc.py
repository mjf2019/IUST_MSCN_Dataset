"""Leakage-controlled OOF meta-stacking for the fixed CDR-MLC expert bank."""
...
def fit_meta_stacker(source, config: MetaStackConfig):
    ...
    # The final model preserves the original paper KMeans and expert design.
    final = fit_fixed_cdr(source, config.window, config.random_state, config.expert_trees)
    helper = LearnedRouterConfig(
        window=config.window, expert_trees=config.expert_trees,
        random_state=config.random_state,
    )
    preliminary = _refit_experts_with_frozen_router(final, split["expert"], helper)
