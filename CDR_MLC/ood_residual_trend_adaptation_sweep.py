"""Adaptation sweep including OOD-aware residual trend meta-stacking.
...
from ood_residual_trend_meta_stacker import (
    ...
    predict_all as predict_ood_residual,
)
...
def main():
    ...
    parser.add_argument(
        "--output", type=Path,
        default=root / "outputs/ood_residual_trend_adaptation",
    )
    parser.add_argument(
    ...
    residual_config = OODResidualConfig(
        window=args.window,
        congestion_window=args.congestion_window,
        router_candidates=tuple(args.congestion_features),
        context_features=tuple(args.congestion_features),
        ...
        for fraction in fractions:
            ...
            prediction_series = {
                ...
                **as_series(trend_result, "TrendSelected_"),
                "R-Meta": pd.Series(
                    residual_result[
                        "CDR_MLC_ood_residual_meta_stacker"
                    ],
            ...
            audits[run_key] = {
                ...
                "residual_selected_router_features": list(
                    residual_model["selected_router_features"]
                ),
                ...
                "residual_partition_rows": residual_model[
                    "partition_rows"
                ],
                "residual_leakage_control": residual_model[
                    "leakage_control"
                ],
            }
    ...
    summary.to_csv(
        args.output / "ood_residual_trend_adaptation_summary.csv",
        index=False,
    )
    manifest = {
        "protocol": (
        ...
        "ood_residual_policy": {
            ...
            "guards": [
                "meta confidence", "meta margin",
                "KMeans distance OOD rejection"
            ],
    ...
    (args.output / "ood_residual_trend_adaptation_manifest.json").write_text(
