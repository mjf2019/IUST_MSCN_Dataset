"""Evaluate the level-balanced meta stacker on one immutable test tail.
...
def build_fixed_test_protocol(data, source_level, adaptation_fraction,
                              test_fraction):
    ...
    for level in ("Medium", "High"):
        ...
        if level == source_level:
            audit.append({
                ...
                "development_rows": len(level_data),
            })
            continue
...
def main():
    ...
    parser.add_argument("--output", type=Path, default=root / "outputs/meta_stacked_fixed_test")
    ...
    for fraction in args.fractions:
        ...
        for source_level, targets in grouped.items():
            development, fixed_tests, protocol_audit = build_fixed_test_protocol(
            ...
            (source_dir / "fixed_test_meta_audit.json").write_text(
    ...
    manifest = {
        ...
        "audits": audits,
        "leakage_control": (
            "The final target tail is immutable across adaptation fractions. "
            "Calibration prefixes and temporal training partitions never include it."
        ),
    }
